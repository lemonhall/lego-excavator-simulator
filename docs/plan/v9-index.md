...
| M3 | 稳定目标场 | 默认 LDraw 实例不少于 9；距离拉开；不伪造社区资产；避免重型实例导致卡顿 | `npm test -- src/game/app.test.ts src/game/communityModels.test.ts`; E2E default population | todo |
| M4 | AI 预留与车辆巡逻 | enemy 纯逻辑保留给真实社区资产；运行时不生成手捏敌人；vehicle 类目标无人驾驶时巡逻且轮子旋转 | `npm test -- src/game/enemies.test.ts src/game/app.test.ts`; E2E vehicle assertions | todo |
| M5 | 碎片回收与性能 | detached 碎片短暂保留后回收；debug 计数可观测；projectile/tracer 上限生效 | `npm test -- src/game/cleanup.test.ts src/game/weapon.test.ts`; E2E cleanup assertions | todo |
...
- 反作弊条款：不能只提高子弹亮度或改名字；必须移除 sphere bead 视觉、默认目标数提升到 9+、不生成手捏 enemy 视觉、有车辆巡逻、有碎片 TTL 回收计数。
