...
提供一个可审计、离线、可测试的砖块级 LDraw 社区模型浏览和投放流程。玩家按 `B` 打开右侧 panel，点击本地精选 `.ldr/.mpd` 模型，将模型加载到农场场景中，并能驾驶挖掘机撞碎它。
...
- 新增 `public/community-models/manifest.json`，定义本地精选 LDraw 模型包。
- 新增 `public/community-models/ldraw/*.mpd`，存放真实 packed LDraw 资产。
- 新增 `src/game/communityModels.ts`，封装 manifest 类型、加载、校验、LDraw 文本分析、Three.js `LDrawLoader` 加载和实例状态。
...
- 不接受 glTF/OBJ/STL 等普通 mesh 模型。
- 不接受项目自编 procedural sample 冒充社区模型。
...
1. `npm test -- src/game/communityModels.test.ts` exit 0，并覆盖：
   - manifest 至少 3 个模型。
   - 每个模型有 `id/name/category/format/sourceKind/sourceUrl/license/attribution/file`。
   - 每个模型引用本地 `.ldr/.mpd`。
   - 单元测试拒绝 hand-authored procedural sample。
   - 每个本地 LDraw 文件包含类型 1 零件引用和 `.dat` 零件数据。
...
3. `npm run e2e -- tests/e2e/game.spec.ts --project=desktop-chrome --grep "spawned community model"` exit 0，并覆盖：
   ...
   - `visiblePartCount > 0`。
   - debug 显示 `sourceKind` 和 `modelFormat === "ldraw"`。
...
- `public/community-models/ldraw/*.mpd`
...
2. TDD Red：新增 `communityModels.test.ts`，验证 manifest、授权字段、LDraw 文件和反作弊规则，运行到红。
3. TDD Green：实现本地 manifest 加载、类型校验、LDraw 文本分析和 `LDrawLoader` 实例加载。
...
- **授权风险**：v7 仅使用可追溯来源的 packed LDraw examples；manifest 必须展示 sourceUrl、license 和 attribution，不宣称包含第三方付费模型。
- **格式风险**：v7 只支持 `.ldr/.mpd` LDraw 文本；普通 mesh 和 procedural JSON 不能通过验收。
